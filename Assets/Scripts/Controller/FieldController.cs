...
public class FieldController : MonoBehaviour
{
		...
		for (int i = 0; i < playCards.Count; i++) {
			...
			if (card.pile == PlayCard.Pile.hand) {
				...
				if (!cardCtrl.IsMoveAnimating ()) {
					...
					if (card.owner == 0) {
						to = new Vector3 (card.pos * 1.5f, 0.2f, 0);
					}
					if (card.owner == 1) {
						to = new Vector3 (card.pos * -1.5f, 0.2f, 0);
					}
	...
	{
		if (playerIndex < 0 || playerIndex >= GameManager.Get ().players.Length) {
			return;
		}
	...
	public void FadeBanner (int playerIndex)
	{
		...
		if (!GameManager.Get ().running || !GameManager.Get ().setUp) {
			return;
		}
...
/**/
		...
		if (libFx.selectorPile == PlayCard.Pile.none) {
			return;
		}

		if (card.pile != PlayCard.Pile.hand && card.pile != PlayCard.Pile.field) {
			return;
		}
	...
	public void GameOver (int player)
	{
		...
		if (player == GameManager.Get().localPlayerId)
		{
